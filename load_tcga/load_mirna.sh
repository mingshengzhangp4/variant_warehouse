...
#### wget http://gdac.broadinstitute.org/runs/stddata__2015_06_01/data/BRCA/20150601/gdac.broadinstitute.org_BRCA.Merge_mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.Level_3.2015060100.0.0.tar.gz

{
wget -nv -P ${path_downloaded}  http://gdac.broadinstitute.org/runs/stddata__${DATE}/data/${TUMOR}/${DATE_SHORT}/gdac.broadinstitute.org_${TUMOR}.Merge_mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.Level_3.${DATE_SHORT}00.0.0.tar.gz

} ||{
echo "This file does not exist:"
echo http://gdac.broadinstitute.org/runs/stddata__${DATE}/data/${TUMOR}/${DATE_SHORT}/gdac.broadinstitute.org_${TUMOR}.Merge_mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.Level_3.${DATE_SHORT}00.0.0.tar.gz
echo "exiting ..."
exit 1
}

  tar -zxvf ${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.Level_3.${DATE_SHORT}00.0.0.tar.gz --directory ${path_downloaded}/

##  create intermediate tsv files for easy loading  ##
  
MIRNAseqFile_original=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.Level_3.${DATE_SHORT}00.0.0/${TUMOR}.mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.data.txt
  
MIRNAseqFile_rpmm=${cwd}/mirna_data.tsv
MIRNAseq_probeFile=${cwd}/mirna_probe.tsv
samplesFile=${cwd}/samples.tsv
unsorted_sampleFile=${cwd}/usamples.tsv

MIRNAseq_geneFile=${cwd}/genes.tsv


# write ${MIRNAseqFile_rpmm}, ${MIRNAseq_probeFile}
python ${cwd}/MIRNAseq_parser.py ${MIRNAseqFile_original} ${gene_file} ${cwd}

 
# output must be one line per sample for 'input' operator to work, thus OFS='\n'
cat ${MIRNAseqFile_rpmm} |head -1|awk -F'\t' -v OFS='\n' '{ for (i=2; i<=NF;i++)  print $i}'|sort|uniq > ${samplesFile} 
        ...
        cross_join(
            ...
            index_lookup(
              ...
              substitute(
                redimension(
                  ...
                  <probe_name:string null>
                  [illuninahiseq_mirnaseq_probe_id=0:*, 1000000,0]
                  ),
                build(<subval:string>[i=0:0,1,0], '_')
                ) as B,
              LB.probe_name,
          ...
          aggregate(
            apply(
              ...
              pid,
              illuninahiseq_mirnaseq_probe_id),
            max(pid) as mpid
            )
          ),
...
MAX_VERSION=`iquery -ocsv -aq "aggregate(versions(mirna_probe_index), max(version_id))"|tail -n 1`
iquery -anq "remove_versions(mirna_probe_index, $MAX_VERSION)"
...
insert(
  redimension(
    apply(
      ...
      genomic_start, 0,
      genomic_end, 0,
      illuminahiseq_mirnaseq_probe_id, probe_id
      ),
    TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_PROBE_STD
    ),
  TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_PROBE_STD
...
MAX_VERSION=`iquery -ocsv -aq "aggregate(versions(TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_PROBE_STD), max(version_id))"|tail -n 1`
iquery -anq "remove_versions(TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_PROBE_STD, $MAX_VERSION)"


column_no=`cat ${MIRNAseqFile_rpmm}|awk -F'\t' '{print NF}' |head -n 1|awk '{print $1}'`
...
iquery -anq "create temp array mirna_LOAD_BUF
...
store(
 parse(
  split('${MIRNAseqFile_rpmm}', 'header=2', 'lines_per_chunk=${CHUNK_SIZE}'),
      ...
      index_lookup(
        apply(
          index_lookup(
            ...
            mirna_probe_index,
            G.expression,
            illuminahiseq_mirnaseq_probe_id
            ),
          ttn, '${TUMOR}',
...
MAX_VERSION=`iquery -ocsv -aq "aggregate(versions(TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_STD), max(version_id))"|tail -n 1`
iquery -anq "remove_versions(TCGA_${DATE}_ILLUMINAHISEQ_MIRNASEQ_STD, $MAX_VERSION)"
