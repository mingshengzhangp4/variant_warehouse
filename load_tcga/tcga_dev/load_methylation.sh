...
select yn in "yes" "no"; do
  ...
  METHYLATIONFile=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/${TUMOR}.rnaseqv2__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.data.txt
  
  samplesFile=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/samples.tsv
  
  Entrez_samplesFile=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/Entrez_samples.tsv
  
  Entrez_geneList=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/Entrez_geneList.tsv
  
  RNAexpr=${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/RNAexpression.tsv
  
  
 # output must be one line per sample for 'input' operator to work, thus OFS='\n'
 cat ${METHYLATIONFile} |head -1|awk -F'\t' -v OFS='\n' '{ for (i=2; i<=NF;i++)  print $i}' > ${samplesFile} 
 
 cat ${METHYLATIONFile}|awk 'NR>2'|awk -F'|' '{ for (i=2; i<=NF; i++) print $i}' > ${Entrez_samplesFile}
...
TCGA_${DATE}_GENE_STD)"
      ...
      for file in `ls ${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0 | grep -i ${TUMOR}`; do
          column_no=`cat ${path_downloaded}/gdac.broadinstitute.org_${TUMOR}.Merge_methylation__humanmethylation450__jhu_usc_edu__Level_3__within_bioassay_data_set_function__data.Level_3.${DATE_SHORT}00.0.0/$file | awk -F'\t' '{print NF}' |head -n 1|awk '{print $1}'`
