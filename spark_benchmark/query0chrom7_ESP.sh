...
unzipvcffile=ESP6500SI-V2-SSA137.GRCh38-liftover.chr7.snps_indels.vcf
suffix=.gz
...
echo "The HDFS PUT process took $DIFFHDFS seconds" >> resultquery1chrom7_ESP.txt

DIFFADAM=$(( $ENDADAM - $STARTADAM ))
echo "The ADAM process took $DIFFADAM seconds" >> resultquery1chrom7_ESP.txt

END=$(date +%s)
DIFF=$(( $END - $START ))
echo "The whole process took $DIFF seconds" >> resultquery1chrom7_ESP.txt
